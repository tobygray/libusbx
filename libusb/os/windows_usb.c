...
static int winusbx_submit_control_transfer(int sub_api, struct usbi_transfer *itransfer)
{
	...
	wfd = usbi_create_fd(winusb_handle, RW_READ);
...
static int winusbx_submit_bulk_transfer(int sub_api, struct usbi_transfer *itransfer)
{
	...
	wfd = usbi_create_fd(winusb_handle, IS_XFERIN(transfer) ? RW_READ : RW_WRITE);
	// Always use the handle returned from usbi_create_fd (wfd.handle)
	if (wfd.fd < 0) {
		return LIBUSB_ERROR_NO_MEM;
	}
...
static int hid_submit_control_transfer(int sub_api, struct usbi_transfer *itransfer)
{
	...
	// Always use the handle returned from usbi_create_fd (wfd.handle)
	wfd = usbi_create_fd(hid_handle, RW_READ);
	if (wfd.fd < 0) {
		return LIBUSB_ERROR_NOT_FOUND;
	}
...
static int hid_submit_bulk_transfer(int sub_api, struct usbi_transfer *itransfer) {
	...
	wfd = usbi_create_fd(hid_handle, direction_in?RW_READ:RW_WRITE);
	// Always use the handle returned from usbi_create_fd (wfd.handle)
	if (wfd.fd < 0) {
		return LIBUSB_ERROR_NO_MEM;
	}
